...
public final class Criticals extends Module {
    ...
    private final ModeSetting mode = new ModeSetting("Mode", "Watchdog", "Watchdog", "Packet", "Dev", "Verus","Syuu","NoGround");
    private final BooleanSetting NoGroundSmartMode = new BooleanSetting("Smart",false);
    private final NumberSetting syuutick = new NumberSetting("SyuuTick",2,20,0,1);
    ...
    public Criticals() {
        ...
        syuutick.addParent(mode,m->m.is("Syuu"));
        this.addSettings(mode, watchdogMode, syuutick,delay,NoGroundSmartMode);
    }
    ...
    public void onPacketReceiveEvent(PacketReceiveEvent event){
        if (mode.is("NoGround")){
            if (KillAura.attacking && event.getPacket() instanceof C03PacketPlayer){
    ...
    public void onMotionEvent(MotionEvent e) {
        ...
        switch (mode.getMode()) {
            case "Watchdog":
                ...
                if (e.isPre() && watchdogMode.is("Edit") && !YolBi.INSTANCE.isEnabled(Flight.class) && !Step.isStepping && KillAura.attacking) {
                    if (e.isOnGround()) {
                        groundTicks++;
                        if(groundTicks > 2) {
                            stage = !stage;
            ...
            case "Syuu":
                if (e.isPre()  && !YolBi.INSTANCE.isEnabled(Flight.class) && !Step.isStepping && KillAura.attacking) {
                    if (e.isOnGround()) {
                        groundTicks++;
                        if(groundTicks > syuutick.getValue()) {
                            stage = !stage;
                            e.setY(e.getY() + (stage ? 0.015 : 0.01) - Math.random() * 0.0001);
                            e.setOnGround(false);
                        }
