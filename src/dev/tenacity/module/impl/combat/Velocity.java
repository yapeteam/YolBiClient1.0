...
public class Velocity extends Module {
    ...
    private final ModeSetting mode = new ModeSetting("Mode", "Packet", "Packet", "Matrix", "Tick", "Stack", "C0F Cancel","HypixelLegit");
    ...
    private final BooleanSetting cancelHorizontal = new BooleanSetting("cancelHorizontal",false);
    private final BooleanSetting hideY = new BooleanSetting("HideY with killaura",false);
    private final BooleanSetting onlyHit = new BooleanSetting("hideY afterHit",false);

    //private final BooleanSetting cancelVertical = new BooleanSetting("cancelVertical",false);
    ...
    public Velocity() {
        ...
        Setting.addParent(mode,m->m.is("HypixelLegit"),staffCheck,vertical,cancelHorizontal);
        onlyHit.addParent(hideY,r->r.isEnabled());
        this.addSettings(mode, horizontal, vertical, chance, onlyWhileMoving, staffCheck,cancelHorizontal,hideY,onlyHit);
    }
    @Override
    public void onTickEvent(TickEvent event) {
        ...
        if (hideY.isEnabled() && !mc.gameSettings.keyBindJump.isKeyDown() && (this.packet instanceof S12PacketEntityVelocity||!onlyHit.isEnabled() ) &&KillAura.attacking) {
        ...
        switch (mode.getMode()) {
            ...
            case "HypixelLegit":
                if (packet instanceof S12PacketEntityVelocity){
                    ...
                    this.packet = packet;
                    if (mc.thePlayer != null && s12.getEntityID() == mc.thePlayer.getEntityId()){
                        int Y = ((S12PacketEntityVelocity) packet).getMotionY();
                        if (cancelHorizontal.isEnabled()){
                            //s12.motionY *= vertical.getValue() / 100.0;
                            //Y = (int) (Y * vertical.getValue()/800000.0);
                            mc.thePlayer.motionY= Y * vertical.getValue()/800000.0;
                            //mc.thePlayer.motionY=Y;
                            e.cancel();


                            //((S12PacketEntityVelocity) packet).motionY=
                            //mc.thePlayer.motionX=((S12PacketEntityVelocity) packet).motionX / 8000.0;
                            //mc.thePlayer.motionZ=((S12PacketEntityVelocity) packet).motionZ / 8000.0;
                        }
