...
import dev.tenacity.utils.FDPMovementUtils.PlaceRotation;
...
public class Scaffold extends Module {
    ...
    private final ModeSetting countMode = new ModeSetting("Block Counter", "YolBi", "None", "YolBi", "Basic", "Polar","Island");
    private final BooleanSetting rotations = new BooleanSetting("Rotations", true);
    private final ModeSetting rotationMode = new ModeSetting("Rotation Mode", "Watchdog", "Watchdog", "NCP", "Back", "45", "Enum", "Down", "0","Custom");

    private final NumberSetting customYawValue = new NumberSetting("CustomYaw",-145,180,-180,1);
    private final NumberSetting customRAD = new NumberSetting("customRAD",1,10,0,0.01);
    private final NumberSetting customPitchValue = new NumberSetting("CustomPitch",82.4f,90f,-90f,0.1);
    private final ModeSetting advancedYawModeValue = new ModeSetting("AdvancedYawRotations ","MoveDirection", "Offset", "Static", "RoundStatic", "Vanilla", "Round", "MoveDirection", "OffsetMove");
    private final ModeSetting advancedPitchModeValue = new ModeSetting("AdvancedPitchRotations ","Static", "Offset", "Static", "Vanilla");
    private final NumberSetting advancedYawOffsetValue = new NumberSetting("AdvancedOffsetYaw",-15,180,-180,1);
    private final NumberSetting advancedYawMoveOffsetValue = new NumberSetting("AdvancedMoveOffsetYaw",-15,180,-180,1);
    private final NumberSetting advancedYawStaticValue = new NumberSetting("AdvancedStaticYaw",145,180,-180,1);
    private final NumberSetting advancedYawRoundValue = new NumberSetting("AdvancedYawRoundValue",45,180,-180,1);
    private final NumberSetting advancedPitchOffsetValue = new NumberSetting("AdvancedOffsetPitch",-0.4f,90f,-90f,0.1);
    private final NumberSetting advancedPitchStaticValue = new NumberSetting("AdvancedStaticPitch",82.4f,90f,-90f,0.1);
    ...
    private boolean pre;
    public static boolean kadisable=false;
    ...
    public Scaffold() {
        ...
        this.addSettings(countMode, rotations, rotationMode,
                customYawValue, customPitchValue,customRAD,
                advancedYawModeValue,advancedPitchModeValue,advancedYawOffsetValue,advancedYawMoveOffsetValue,advancedYawStaticValue,advancedYawRoundValue,advancedPitchOffsetValue,advancedPitchStaticValue,
                placeType, keepYMode, sprintMode, towerMode, swingMode, delay, timer,
        ...
        customYawValue.addParent(rotationMode,m ->m.is("Custom"));
        customPitchValue.addParent(rotationMode,m ->m.is("Custom"));
        customRAD.addParent(rotationMode,m ->m.is("Custom"));
        advancedYawModeValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedPitchModeValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedYawOffsetValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedYawMoveOffsetValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedYawStaticValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedYawRoundValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedPitchOffsetValue.addParent(rotationMode,m ->m.is("Advanced"));
        advancedPitchStaticValue.addParent(rotationMode,m ->m.is("Advanced"));
        ...
        if (e.isPre()) {
            ...
            if (rotations.isEnabled()) {
                ...
                switch (rotationMode.getMode()) {
                    ...
                    case "Custom":
                        e.setRotations(mc.thePlayer.rotationYaw + customYawValue.getValue().floatValue()+new Random().nextFloat()* customRAD.getValue().floatValue(), customPitchValue.getValue().floatValue()+new Random().nextFloat()* customRAD.getValue().floatValue());
                        break;

                    case "Advanced":
                        float advancedYaw = 0f;
                        float advancedPitch = 0f;
                        //neighbor = blockPosition.offset(side);
                        switch (advancedYawModeValue.getMode().toLowerCase()){
                            case "offset":
                                //Rotation rotation=new Rotation();
                                //PlaceRotation placeRotation = new PlaceRotation(PlaceInfo(neighbor, side.opposite, hitVec), rotation);
                                //advancedYaw = +advancedYawOffsetValue.getValue().floatValue();
                                break;
                            case "static":
                                advancedYaw = mc.thePlayer.rotationYaw + advancedYawStaticValue.getValue().floatValue();
                                break;
                            case "vanilla":
                                break;
                            case "round":
    ...
    public void renderCounter() {
        ...
        switch (countMode.getMode()) {
            ...
            case "Polar":
                ...
                break;
            case "Island":
                blockWH = heldItem != null ? 15 : -2;
                spacing = 10;
                text = "§l" + countStr + "§r block" + (count != 1 ? "s" : "");
                textWidth = tenacityFont18.getStringWidth(text);

                totalWidth = ((textWidth + blockWH + spacing) + 6) * output*1.2f;
                x = sr.getScaledWidth() / 2f - (totalWidth / 2f);
                //y = sr.getScaledHeight() - (sr.getScaledHeight() / 2f - 20);
                y=5f;
                height = 25;
                ...
                RoundedUtil.drawRound(x, y, totalWidth, height, 10, new Color(0,0,0));
                Color color1;
                if (count >= 32){
                    color1 = new Color(19, 255, 0);
                } else if (count >= 16) {
                    color1 = new Color(255, 206, 0);
                }else {
                    color1 = new Color(255, 7, 7);
                }
                tenacityFont18.drawString(text, x + 3 + blockWH + spacing, y + tenacityFont18.getMiddleOfBox(height) + .5f, -1);
                RoundedUtil.drawRoundOutline(x+67,y+8,10,10,5,0.001f,color1,new Color(255,255,255));

                if (heldItem != null) {
                    RenderHelper.enableGUIStandardItemLighting();
                    mc.getRenderItem().renderItemAndEffectIntoGUI(heldItem, (int) x + 3, (int) (y + 12 - (blockWH / 2)));
                    RenderHelper.disableStandardItemLighting();
                }
                RenderUtil.scissorEnd();
                break;
