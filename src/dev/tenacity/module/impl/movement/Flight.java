...
import static dev.tenacity.utils.player.MovementUtils.isMoving;
...
public final class Flight extends Module {
    ...
    private final ModeSetting mode = new ModeSetting("Mode", "Watchdog", "Zonecraft", "Watchdog", "Vanilla", "AirWalk", "Viper", "Verus", "Minemen", "Old NCP", "Slime", "Custom", "Packet", "Minemora", "Vulcan","Blockdrop");
    ...
    private double startyaw = 0f;
    private double startpitch = 0f;
    ...
    private final NumberSetting hSpeedValue = new NumberSetting("horizontalspeed",1,10,0,0.1);
    private final NumberSetting vSpeedValue = new NumberSetting("verticalSpeed",0.42,10,0,0.1);
    ...
    public Flight() {
        ...
        hSpeedValue.addParent(mode,m -> m.is("Blockdrop"));
        vSpeedValue.addParent(mode,m -> m.is("Blockdrop"));
        ...
        this.addSettings(mode, teleportDelay, teleportLength, motionY, damage, damageMode, speed, speedAmount, timerAmount, horizontalSpeed, verticalSpeed, viewBobbing, antiKick,vSpeedValue,hSpeedValue);
    }
    ...
    public void onMoveEvent(MoveEvent e) {
        switch (mode.getMode()) {
            case "Vanilla":
                e.setSpeed(isMoving() ? horizontalSpeed.getValue().floatValue() : 0);
        ...
        switch (mode.getMode()) {
            case "Watchdog":
                if (e.isPre()) {
                    ...
                    stage++;
                    if(stage == 1) {
                        final double x = e.getX() + -sin(Math.toRadians(mc.thePlayer.rotationYaw)) * 7.99;
            ...
            case "Old NCP":
                if (hasDamaged) {
                    ...
                    if (!isMoving() || mc.thePlayer.isCollidedHorizontally) moveSpeed = baseSpeed;
            ...
            case "Packet":
                if(e.isPre()) {
                    mc.thePlayer.motionY = 0;
                    if(isMoving() && mc.thePlayer.ticksExisted % teleportDelay.getValue().intValue() == 0) {
                        final double x = e.getX() + -sin(Math.toRadians(mc.thePlayer.rotationYaw)) * teleportLength.getValue().intValue();
    ...
    private void strafe(Double speed) {
        if (!isMoving()) return;
        mc.thePlayer.motionX = -sin(Rotation.direction()) * speed;
        mc.thePlayer.motionZ = cos(Rotation.direction()) * speed;
    }
    ...
    public void onDisable() {
        if (mode.is("Blockdrop")){
            //BlockdropFly()
            //BlockdropFly.class.
            //BlockdropFly.onEnable();
            //Module(new BlockdropFly())
        }
