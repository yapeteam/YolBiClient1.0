...
public class HUDMod extends Module {
    ...
    private final ModeSetting watermarkMode = new ModeSetting("Watermark Mode", "YolBi", "YolBi", "Plain Text", "Neverlose", "Tenasense", "Tenabition", "Logo", "None","Yolose");
    ...
    public static final ColorSetting rainbow = new ColorSetting("rainbow", new Color(0));
    public static final NumberSetting RainBowOffset = new NumberSetting("offset", 30, 180, 0, 1);
    public static final ModeSetting theme = Theme.getModeSetting("Theme Selection", "YolBi");
    ...
    private static final MultipleBoolSetting disableButtons = new MultipleBoolSetting(
            "Disable Buttons",
    ...
    public HUDMod() {
        ...
        color2.addParent(theme, modeSetting -> modeSetting.is("Custom Theme"));
        rainbow.setRainbow(true);
        rainbow.setOnlyRainBow(true);
        rainbow.getRainbow().setSpeed(1);
        rainbow.addParent(theme, modeSetting -> modeSetting.is("RainBow"));
        RainBowOffset.addParent(theme, modeSetting -> modeSetting.is("RainBow"));
        this.addSettings(clientName, watermarkMode, theme, color1, color2, rainbow, RainBowOffset, customFont, infoCustomization, hudCustomization, disableButtons);
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        if (e.isBloom()) {
            ...
            switch (watermarkMode.getMode()) {
                case "Logo":
                    float WH = 110 / 2f;
                    float textWidth = FontUtil.getFromCustomFont(tenacityBoldFont32).getStringWidth(finalName);

                    GL11.glEnable(GL11.GL_SCISSOR_TEST);
                    RenderUtil.scissor(10, 7, 13 + WH + textWidth + 5, WH);

                    FontUtil.getFromCustomFont(tenacityBoldFont32).drawString(finalName, ((13 + WH) - textWidth) + (textWidth * fadeInText.getOutput().floatValue()), 8 + FontUtil.getFromCustomFont(tenacityBoldFont32).getMiddleOfBox(WH), ColorUtil.applyOpacity(glow ? -1 : 0, fadeInText.getOutput().floatValue()));
                    GL11.glDisable(GL11.GL_SCISSOR_TEST);


                    GradientUtil.applyGradientCornerLR(27, 23, WH - 28, WH - 28, 1, clientColors.getSecond(), clientColors.getFirst(), () -> {
                        mc.getTextureManager().bindTexture(new ResourceLocation("YolBi/watermarkBack.png"));
                ...
                case "YolBi":
                    float xVal = 6f;
                    float yVal = 6f;
                    float versionWidth = FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(YolBi.VERSION);
                    float versionX = xVal + FontUtil.getFromCustomFont(tenacityBoldFont40).getStringWidth(finalName);
                    float width = version ? (versionX + versionWidth) - xVal : FontUtil.getFromCustomFont(tenacityBoldFont40).getStringWidth(finalName);


                    RenderUtil.resetColor();
                    GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                        RenderUtil.setAlphaLimit(0);
                        FontUtil.getFromCustomFont(tenacityBoldFont40).drawString(finalName, xVal, yVal + 10, 0);
                        if (version) {
                            FontUtil.getFromCustomFont(tenacityFont16).drawString(YolBi.VERSION, versionX, yVal, 0);
                        }
                    });
                ...
                case "Plain Text":
                    ...
                    if (customFont.isEnabled()) {
                        fr = FontUtil.getFromCustomFont(tenacityFont24);
                    }
                    ...
                    break;
                case "Neverlose":
                    AbstractFontRenderer t18 = FontUtil.getFromCustomFont(tenacityFont18);
                ...
                case "Tenasense":
                    ...
                    Gui.drawRect2(x, y, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 7, 18.5, glow ? new Color(59, 57, 57).getRGB() : Color.BLACK.getRGB());
                    break;
            }
        }
    }
    ...
    public void onRender2DEvent(Render2DEvent e) {
        if (alphaAni.isDone()){
            if (alphaAni.getDirection() == Direction.BACKWARDS) alphaAni.setDirection(Direction.FORWARDS);
            else if (alphaAni.getDirection() == Direction.FORWARDS) alphaAni.setDirection(Direction.BACKWARDS);
        }
        ScaledResolution sr = new ScaledResolution(mc);
        Pair<Color, Color> clientColors = getClientColors();
        String name = YolBi.NAME;
        PostProcessing postProcessing = YolBi.INSTANCE.getModuleCollection().getModule(PostProcessing.class);
        ...
        version = name.equalsIgnoreCase(YolBi.NAME);

        String finalName = get(name);
        //String intentInfo = YolBi.INSTANCE.getIntentAccount().username;
        String intentInfo = mc.getSession().getUsername();
        ...
        switch (watermarkMode.getMode()) {
            case "Logo":
                ...
                FontUtil.getFromCustomFont(tenacityBoldFont32).drawString(finalName, ((13 + WH) - textWidth) + (textWidth * fadeInText.getOutput().floatValue()), 8 + FontUtil.getFromCustomFont(tenacityBoldFont32).getMiddleOfBox(WH), ColorUtil.applyOpacity(-1, .7f * fadeInText.getOutput().floatValue()));
                GL11.glDisable(GL11.GL_SCISSOR_TEST);
                ...
                RenderUtil.color(-1);
                GLUtil.startBlend();
                mc.getTextureManager().bindTexture(new ResourceLocation("YolBi/watermarkT.png"));
                Gui.drawModalRectWithCustomSizedTexture(7, 7, 0, 0, WH, WH, WH, WH);
                break;
            case "YolBi":
                float xVal = 5;
                ...
                GradientUtil.applyGradientHorizontal(xVal + spacing, yVal + spacing, width + spacing,
                        20, 1, darkerColors.getFirst(), darkerColors.getSecond(), () -> {
                            RenderUtil.setAlphaLimit(0);
                            FontUtil.getFromCustomFont(tenacityBoldFont40).drawString(finalName1, xVal + spacing, yVal + spacing + 10, 0);
                            if (version) {
                                FontUtil.getFromCustomFont(tenacityFont16).drawString(YolBi.VERSION, versionX + (spacing / 2f), yVal + (spacing / 2f), 0);
                            }
                        });
                ...
                GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                    RenderUtil.setAlphaLimit(0);
                    FontUtil.getFromCustomFont(tenacityBoldFont40).drawString(finalName1, xVal, yVal + 10, 0);
                    if (version) {
                        FontUtil.getFromCustomFont(tenacityFont16).drawString(YolBi.VERSION, versionX, yVal, 0);
                    }
                });
            ...
            case "Plain Text":
                ...
                break;
            case "Neverlose":
                AbstractFontRenderer m22 = neverloseFont.size(22), t18 = FontUtil.getFromCustomFont(tenacityFont18);
                String str = String.format(" §8|§f %s fps §8|§f %s §8|§f %s",
                        Minecraft.getDebugFPS(), intentInfo,
                        mc.isSingleplayer() || mc.getCurrentServerData() == null ? "singleplayer" : mc.getCurrentServerData().serverIP);
                name = name.toUpperCase();
            ...
            case "Yolose":
                ...
                AbstractFontRenderer m221 = neverloseFont.size(22), t181 = FontUtil.getFromCustomFont(tenacityFont18);
                String str1 = String.format(" §8|§f %s fps §8|§f %s §8|§f %s",
                        Minecraft.getDebugFPS(), intentInfo,
                        mc.isSingleplayer() || mc.getCurrentServerData() == null ? "singleplayer" : mc.getCurrentServerData().serverIP);
                name = name.toUpperCase();
                float nw1 = m221.getStringWidth(name);
                if (alphaAni.getOutput() != 0){
                    color11 = ColorUtil.applyOpacity(ColorUtil.interpolateColorC(clientColors.getFirst(), clientColors.getSecond(), .65f), (float) (.7f * alphaAni.getOutput()));

                }


                RoundedUtil.drawRoundOutline(4, 4.0F, nw1 + t181.getStringWidth(str1) + 6f, t181.getHeight() + 6,8,0.01f,new Color(0,0,0,1),color11);
                tenacityFont18.drawString(str1, 7.5F + nw1, 8.5F, color11.getRGB());
                m221.drawString(name, 7.5F, 9, color11.getRGB());
                m221.drawString(name, 7, 8.5F, color11.getRGB());
                break;
            case "Tenasense":
                String text = "§ftena§rsense§f" + " - " + intentInfo + " - " + (mc.isSingleplayer() ? "singleplayer" : mc.getCurrentServerData().serverIP) + " - "
                        + PingerUtils.getPing() + "ms ";
                ...
                Gui.drawRect2(x, y, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 7, 18.5, new Color(59, 57, 57).getRGB());

                Gui.drawRect2(x + 2.5, y + 2.5, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 2, 13, new Color(23, 23, 23).getRGB());

                // Top small bar
                Gui.drawRect2(x + 1, y + 1, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 5, .5, lineColor);

                // Bottom small bar
                Gui.drawRect2(x + 1, y + 17, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 5, .5, lineColor);

                // Left bar
                Gui.drawRect2(x + 1, y + 1.5, .5, 16, lineColor);

                // Right Bar
                Gui.drawRect2((x + 1.5) + FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 4, y + 1.5, .5, 16, lineColor);

                // Lowly saturated rainbow bar
                GradientUtil.drawGradientLR(x + 2.5f, y + 14.5f, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 2, 1, 1, clientColors.getFirst(), clientColors.getSecond());

                // Bottom of the rainbow bar
                Gui.drawRect2(x + 2.5, y + 16, FontUtil.getFromCustomFont(tenacityFont16).getStringWidth(text) + 2, .5, lineColor);
                FontUtil.getFromCustomFont(tenacityFont16).drawString(text, x + 4.5f, y + 5.5f, clientColors.getSecond().getRGB());
                break;
            case "Tenabition":
    ...
    private void drawBottomRight() {
        ...
        String text = YolBi.VERSION + " - " + (customFont.isEnabled() ? "" : "§l") + YolBi.RELEASE.getName() + "§r";
        //显示用户名字
        //show Username here
    ...
    public static boolean isRainbowTheme() {
        return theme.is("Custom Theme") && (color1.isRainbow() || color2.isRainbow());
    }
    ...
    public static void updateButtonStatus() {
        for (ModuleButton mb : ModuleButton.values()) {
            mb.getButton().enabled = YolBi.INSTANCE.getModuleCollection().getModule(mb.getModule()).isEnabled();
        }
    }
    ...
    public static void handleActionPerformed(GuiButton button) {
        for (ModuleButton mb : ModuleButton.values()) {
            if (mb.getButton() == button) {
                Module m = YolBi.INSTANCE.getModuleCollection().getModule(mb.getModule());
                if (m.isEnabled()) {
                    m.toggle();
                }
                break;
