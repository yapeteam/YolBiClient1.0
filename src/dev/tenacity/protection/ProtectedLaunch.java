...
import dev.tenacity.module.impl.ghost.Clach;
import dev.tenacity.module.impl.ghost.ItemSpoof;
...
import dev.tenacity.ui.login.LoginGUI;
...
public class ProtectedLaunch {
    ...
    public static void start() {
        ...
        YolBi.INSTANCE.setIntentAccount(GetUserInfo.loginFailure);
        YolBi.INSTANCE.setModuleCollection(new ModuleCollection());

        YolBi.INSTANCE.setEventProtocol(new EventProtocol());
        YolBi.INSTANCE.setCloudDataManager(new CloudDataManager());
        YolBi.INSTANCE.setExecutorService(Executors.newSingleThreadExecutor());
        YolBi.INSTANCE.setSideGui(new SideGUI());
        YolBi.INSTANCE.setSearchBar(new SearchBar());
        ...
        modules.put(KeepContainer.class, new KeepContainer());
        modules.put(Crasher.class, new Crasher());
        modules.put(ClientSpoofer.class, new ClientSpoofer());
        ...
        modules.put(BlockdropFly.class, new BlockdropFly());
        modules.put(VelocityFly.class, new VelocityFly());
        modules.put(BowJump.class, new BowJump());
        ...
        modules.put(HandBow.class, new HandBow());
        ...
        modules.put(MotionBlurMod.class, new MotionBlurMod());
        modules.put(Macos.class, new Macos());
        modules.put(ScreenRecorder.class, new ScreenRecorder());
        modules.put(RenderMemory.class,new RenderMemory());
        //Ghost
        modules.put(Clach.class, new Clach());
        modules.put(ItemSpoof.class, new ItemSpoof());

        YolBi.INSTANCE.getModuleCollection().setModules(modules);

        Theme.init();

        YolBi.INSTANCE.setPingerUtils(new PingerUtils());

        YolBi.INSTANCE.setScriptManager(new ScriptManager());
        ...
        YolBi.INSTANCE.setCommandHandler(commandHandler);
        YolBi.INSTANCE.getEventProtocol().register(new BackgroundProcess());

        YolBi.INSTANCE.setConfigManager(new ConfigManager());
        ConfigManager.defaultConfig = new File(Minecraft.getMinecraft().mcDataDir + "/YolBi/Config.json");
        YolBi.INSTANCE.getConfigManager().collectConfigs();
        if (ConfigManager.defaultConfig.exists()) {
            YolBi.INSTANCE.getConfigManager().loadConfig(YolBi.INSTANCE.getConfigManager().readConfigData(ConfigManager.defaultConfig.toPath()), true);
        }

        DragManager.loadDragData();

        YolBi.INSTANCE.setAltManager(new GuiAltManager());

        FFMpegUtil.init();

        //UltraLight.init();
        //YolBi.INSTANCE.setRenderer(UltraLight.getRenderer());
        //YolBi.INSTANCE.HTMLGui = new HTMLGui(YolBi.INSTANCE.getRenderer(), "file://ultralight/public/index.html", 1920, 1080);
        //YolBi.INSTANCE.getEventProtocol().register(new UltralightEvent());

        try {
            YolBi.LOGGER.info("Starting ViaMCP...");
    ...
    private static void downloadDiscordImages() {
        if (YolBi.INSTANCE.getIntentAccount().discord_id != null && !YolBi.INSTANCE.getIntentAccount().discord_id.isEmpty()) {
            IntentAccount intentAccount = YolBi.INSTANCE.getIntentAccount();
            ...
            if (response != null && response.getResponse() == 200) {
                ...
                YolBi.INSTANCE.setDiscordAccount(discordAccount);
            }
        }
    }
