...
public class CustomMainMenu extends GuiScreen {
    ...
    private final List<TextButton> textButtons = new ArrayList<TextButton>() {{
        add(new TextButton("Scripting"));
        add(new TextButton("Discord"));
    }};

    private final ResourceLocation backgroundResource = new ResourceLocation("YolBi/MainMenu/funny.png");
    private final ResourceLocation blurredRect = new ResourceLocation("YolBi/MainMenu/rect-test.png");
    private final ResourceLocation hoverCircle = new ResourceLocation("YolBi/MainMenu/hover-circle.png");

    private static boolean firstInit = false;

    private Thread discordInitThread = null;

    @Override
    public void initGui() {
        if (!firstInit) {
            NetworkingUtils.bypassSSL();
            if (discordInitThread == null) {
                discordInitThread = new Thread(() -> {
                    if (Util.getOSType() == Util.EnumOS.WINDOWS) {
                        YolBi.INSTANCE.setDiscordRPC(new DiscordRPC());
                    }
                    discordInitThread = null;
                });
        ...
        if (mc.gameSettings.guiScale != 2) {
            YolBi.prevGuiScale = mc.gameSettings.guiScale;
            YolBi.updateGuiScale = true;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (animatedOpen) {
            //    tenacityFont80.drawCenteredString("YolBi", width / 2f, height / 2f - 110, Color.WHITE.getRGB());
            //    tenacityFont32.drawString(YolBi.VERSION, width / 2f + tenacityFont80.getStringWidth("YolBi") / 2f - (tenacityFont32.getStringWidth(YolBi.VERSION) / 2f), height / 2f - 113, Color.WHITE.getRGB());
        }
        ...
        ResourceLocation rs = new ResourceLocation("YolBi/MainMenu/circle-funny.png");
            ...
            button.clickAction = () -> {
                switch (button.text) {
                    ...
                    case "Multiplayer":
                        ...
                        break;
                    case "Alt Manager":
                        mc.displayGuiScreen(YolBi.INSTANCE.getAltManager());
                        break;
                    case "Settings":
        ...
        tenacityBoldFont80.drawCenteredString("YolBi", width / 2f, height / 2f - 110, Color.WHITE.getRGB());
        tenacityFont32.drawString(YolBi.VERSION, width / 2f + tenacityBoldFont80.getStringWidth("YolBi") / 2f - (tenacityFont32.getStringWidth(YolBi.VERSION) / 2f), height / 2f - 113, Color.WHITE.getRGB());
        tenacityFont18.drawCenteredString("made by yuxiangll , TIMER_err , Awaf1337", width / 2f, height / 2f - 68, Color.WHITE.getRGB());
    }
