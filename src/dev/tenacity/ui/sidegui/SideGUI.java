...
import dev.tenacity.ui.sidegui.panels.musicpanel.MusicPanel;
...
public class SideGUI implements Screen {
    ...
    public void initGui() {
        if (panels == null) {
            ...
            panels.put("CloudMusic", new MusicPanel());
        }
        if (forms == null) {
            forms = new HashMap<>();
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (focused) {
            ...
            if (panels.get(hotbar.getCurrentPanel()) instanceof MusicPanel) {
                MusicPanel panel = (MusicPanel) panels.get(hotbar.getCurrentPanel());
                panel.keyTyped(typedChar, keyCode);
                if (keyCode == Keyboard.KEY_ESCAPE) {
                    panel.getTextField().textString = "";
                }
            }
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        if (drag == null) return;
        ScaledResolution sr = new ScaledResolution(mc);
        HUDMod hudMod = (HUDMod) YolBi.INSTANCE.getModuleCollection().get(HUDMod.class);
    ...
    public void drawForEffects(boolean bloom) {
        ...
        if (drag == null) return;
        float x =
                MathUtils.interpolateFloat(
                        sr.getScaledWidth(),
                        drag.getX(),
                        openAnimation.getOutput());
        float y = drag.getY();
