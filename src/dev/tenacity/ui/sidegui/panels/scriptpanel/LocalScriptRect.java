...
public class LocalScriptRect implements Screen {
    ...
    public LocalScriptRect(Script script) {
        this.script = script;
        YolBi.INSTANCE.getSideGui().addTooltip(hoverInformation);
            ...
            button.setClickAction(() -> {
                ...
                switch (button.getIcon()) {
                    case FontUtil.UPLOAD:
                        Form uploadForm = YolBi.INSTANCE.getSideGui().displayForm("Upload Script");
                        uploadForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                boolean validScript = YolBi.INSTANCE.getScriptManager().processScriptData(script.getFile());
                                if (validScript) {
