...
public class CloudConfigRect implements Screen {
    ...
    public CloudConfigRect(CloudConfig config) {
        ...
        YolBi.INSTANCE.getSideGui().getTooltips().add(hoverInformation);
                ...
                switch (iconButton.getIcon()) {
                    case FontUtil.LOAD:
                        Multithreading.runAsync(() -> {
                            ...
                            if (YolBi.INSTANCE.getConfigManager().loadConfig(loadData, false)) {
                    ...
                    case FontUtil.SAVE:
                        Multithreading.runAsync(() -> {
                            ...
                            if (YolBi.INSTANCE.getConfigManager().saveConfig(name, saveData)) {
                            ...
                            YolBi.INSTANCE.getSideGui().getTooltips().clear();
                            YolBi.INSTANCE.getSideGui().getConfigPanel().setRefresh(true);
                        });
                        break;
                    case FontUtil.EDIT:
                        Form form = YolBi.INSTANCE.getSideGui().displayForm("Edit Config");
                        ((EditForm) form).setup(config, false);
                        form.setUploadAction((fileName, updatedDescription) -> {
                            Multithreading.runAsync(() -> {

                                String data = YolBi.INSTANCE.getConfigManager().serialize();
