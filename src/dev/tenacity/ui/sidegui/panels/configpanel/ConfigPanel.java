...
public class ConfigPanel extends Panel {
    ...
    private final CarouselButtons carouselButtons = new CarouselButtons("Local", "Cloud");
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> YolBi.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        YolBi.INSTANCE.getCloudDataManager().applyVotes();
        ...
        for (ActionButton button : actionButtons) {
            ...
            button.setClickAction(() -> {
                switch (button.getName()) {
                    case "Upload config":
                        Form uploadForm = YolBi.INSTANCE.getSideGui().displayForm("Upload Config");
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = YolBi.INSTANCE.getConfigManager().serialize();
                                CloudUtils.postOnlineConfig(name, description, server, fileData);
                                YolBi.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
                    case "Save current config":
                        Form saveForm = YolBi.INSTANCE.getSideGui().displayForm("Save Config");
                        saveForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                YolBi.INSTANCE.getConfigManager().saveConfig(name);
                                YolBi.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> YolBi.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }

                if (YolBi.INSTANCE.getCloudDataManager().isRefreshing())
                    return;
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    ...
                    if (filterAuthor) {
                        ...
                        if (!YolBi.INSTANCE.getIntentAccount().username.equals(configAuthor)) continue;
                    }

                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(YolBi.VERSION)) continue;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : YolBi.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(YolBi.VERSION) ? 100 : 0;
