...
public class CloudDataUtils {
    ...
    public static void refreshCloud() {
        if (YolBi.INSTANCE.getSideGui().getPanels() != null) {
            ConfigPanel configPanel = (ConfigPanel) YolBi.INSTANCE.getSideGui().getPanels().get("Configs");
            configPanel.refresh();

            ScriptPanel scriptPanel = (ScriptPanel) YolBi.INSTANCE.getSideGui().getPanels().get("Scripts");
            scriptPanel.refresh();
        }
