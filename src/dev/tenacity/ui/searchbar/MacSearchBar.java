...
@Getter
@Setter
public class MacSearchBar implements Screen {
    ...
    public void keyTyped(char typedChar, int keyCode) {
        ...
        if (GuiScreen.isCtrlKeyDown() && keyCode == Keyboard.KEY_F) {
            ...
            YolBi.INSTANCE.getModuleCollection().getModules().forEach(module -> module.setExpanded(false));
            return;
        }
    ...
    public void drawEffects() {
        ...
        tenacityFont26.drawCenteredString("Do §lCTRL§r+§lF§r to open the search bar", sr.getScaledWidth() / 2f, sr.getScaledHeight() - 75,
                ColorUtil.applyOpacity(new Color(236, 133, 209), (1 * (1 - searchAlpha)) * openAnim));


        RoundedUtil.drawRound(searchField.getXPosition(), searchField.getYPosition(), 200, searchField.getHeight(), 5,
                ColorUtil.applyOpacity(new Color(46, 111, 208), Math.max(hover, focusAnim)));
    }
