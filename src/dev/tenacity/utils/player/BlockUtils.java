...
public class BlockUtils implements Utils {
    ...
    public static boolean isInsideBlock() {
        ...
        while (x2 < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxX) + 1) {
            int y2 = MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().minY);
            while (y2 < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxY) + 1) {
                ...
                while (z2 < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxZ) + 1) {
                    ...
                    if (block != null && !(block instanceof BlockAir) && (boundingBox = block.getCollisionBoundingBox(mc.theWorld, new BlockPos(x2, y2, z2), mc.theWorld.getBlockState(new BlockPos(x2, y2, z2)))) != null && mc.thePlayer.getEntityBoundingBox().intersectsWith(boundingBox)) {
                        return true;
                    }
                    ++z2;
                }
