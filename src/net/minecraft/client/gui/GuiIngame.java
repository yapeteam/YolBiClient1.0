...
public class GuiIngame extends Gui implements Utils {
    Color color1 = ColorUtil.fade(15,2,new Color(76, 120, 255),255);
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        YolBi.INSTANCE.getEventProtocol().handleEvent(new PreRenderEvent());


        PostProcessing postProcessing = (PostProcessing) YolBi.INSTANCE.getModuleCollection().get(PostProcessing.class);
        postProcessing.blurScreen();


        YolBi.INSTANCE.getEventProtocol().handleEvent(new Render2DEvent(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()));
        NotificationsMod notif = YolBi.INSTANCE.getModuleCollection().getModule(NotificationsMod.class);
        if (notif.isEnabled()) {
            notif.render();
        }
        ...
        if (scoreobjective1 != null && YolBi.INSTANCE.isEnabled(ScoreboardMod.class)) {
    ...
    public void renderScoreboardBlur(ScaledResolution scaledRes) {
        ...
        if (objective != null && YolBi.INSTANCE.isEnabled(ScoreboardMod.class)) {
            ...
            for (Score score1 : collection) {
                ...
                if (ScoreboardMod.Round.isEnabled() == false){
    ...
    public void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        if (ScoreboardMod.Round.isEnabled()){
            int allH = 0;
            int i2=1;
            int l = scaledRes.getScaledWidth() - 3 + 2;
            for (Score score2 : collection){
                ++i2;
                allH = this.getScoreboardFontRenderer().getHeight()+allH;

            }
            GaussianBlur.startBlur();
            RoundedUtil.drawRoundOutline(l1-2-3-ScoreboardMod.increase.getValue().floatValue(),j1-i2*this.getScoreboardFontRenderer().getHeight()-3-ScoreboardMod.increase.getValue().floatValue(),ScoreboardMod.increase.getValue().floatValue()+ScoreboardMod.width.getValue().floatValue()+ScoreboardMod.increase.getValue().floatValue() ,ScoreboardMod.increase.getValue().floatValue()+allH+3+this.getScoreboardFontRenderer().getHeight()+ScoreboardMod.increase.getValue().floatValue(),ScoreboardMod.Radious.getValue().floatValue()+3,ScoreboardMod.Outlinethickness.getValue().floatValue(),color,ScoreboardMod.OutlineColor.getColor());
            GaussianBlur.endBlur(ScoreboardMod.Blur.getValue().floatValue(),1);

            RoundedUtil.drawRoundOutline(l1-2-3-ScoreboardMod.increase.getValue().floatValue(),j1-i2*this.getScoreboardFontRenderer().getHeight()-3-ScoreboardMod.increase.getValue().floatValue(),ScoreboardMod.increase.getValue().floatValue()+ScoreboardMod.width.getValue().floatValue()+ScoreboardMod.increase.getValue().floatValue() ,ScoreboardMod.increase.getValue().floatValue()+allH+3+this.getScoreboardFontRenderer().getHeight()+ScoreboardMod.increase.getValue().floatValue(),ScoreboardMod.Radious.getValue().floatValue()+3,ScoreboardMod.Outlinethickness.getValue().floatValue(),color,ScoreboardMod.OutlineColor.getColor());

            //RoundedUtil.drawRound(l1-2,k,l,allH,ScoreboardMod.Radious.getValue().floatValue(),ScoreboardMod.Blur.isEnabled(),color);
        }
    ...
    public AbstractFontRenderer getScoreboardFontRenderer() {
        return ScoreboardMod.customFont.isEnabled() ? FontUtil.tenacityFont20 : this.mc.fontRendererObj;
    }
