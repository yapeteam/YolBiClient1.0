...
public abstract class FFTask<T extends FFOperationBase> implements FFThread {
     ...
     * 读取任务的执行是否完成
     */
    private final Map<FFTaskReadType, FFTaskReadState> readState = new HashMap<>();
    ...
    public void run() {
        ...
        //执行的命令
        String cmd = operationBase.toString().replace("ffmpeg", new File(YolBi.DIRECTORY, "record/ffmpeg").getAbsolutePath());
